import { FailedDependencyError } from '../helpers/customErrors';
  ...
  const schemaValidation: Schema = {
    número_item: (order: orderLine, fileName: string): boolean => {
      ...
      if (!Number.isInteger(order.número_item)) {
        throw new FailedDependencyError(`Error on file ${fileName}! The value of número_item must to be a integer`);
      };

      if (order.número_item < 1) {
        throw new FailedDependencyError(`Error on file ${fileName}! The value of número_item cannot be less than 0`);
      };
    ...
    código_produto: (order: orderLine, fileName: string): boolean => {
      ...
      if (!isAlphanumeric || typeof order.código_produto != 'string') {
        throw new FailedDependencyError(`Error on file ${fileName}! The value of código_produto must to be a alphanumeric`);
      };
    ...
    quantidade_produto: (order: orderLine, fileName: string): boolean => {
      if (typeof order.quantidade_produto != 'number') {
        throw new FailedDependencyError(`Error on file ${fileName}! The value of quantidade_produto must to be a number`);
      };

      if (!Number.isInteger(order.quantidade_produto)) {
        throw new FailedDependencyError(`Error on file ${fileName}! The value of quantidade_produto must to be a integer`);
      };

      if (order.quantidade_produto < 1) {
        throw new FailedDependencyError(`Error on file ${fileName}! The value of quantidade_produto cannot be less than 0`);
      };
    ...
    valor_unitário_produto: (order: orderLine, fileName: string): boolean => {
      ...
      if (!isNumericAndPositive) {
        throw new FailedDependencyError(`Error on file ${fileName}! The value of valor_unitário_produto must to be a numeric and positive`);
      };

      if (splittedValue[1]?.length > 2) {
        throw new FailedDependencyError(`Error on file ${fileName}! The value of valor_unitário_produto cannot exceed two decimal places`);
      };
