    ...
    orders[key].map(orderItem => {
      ...
      if(referringInvoices.length > 0) {
        ...
        if (soma > orderItem.quantidade_produto) {
          throw new FailedDependencyError(`Error! Sum of items on invoices referring to item ${orderItem.número_item} on order ${orderItem.id_pedido} exceed available amount.`);
        };
